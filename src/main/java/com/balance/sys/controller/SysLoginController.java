...
public class SysLoginController {
	...
	public ModelAndView index(HttpServletRequest request, HttpServletResponse response) {
		...
		if (userSession != null) {
			...
			} else if (userSession.getType() == 4) {
				mv.setViewName("redirect:/teach/schedule/my.htm");
			} else {
				mv.setViewName("/index");
				// StatIndex statIndex =
				// statIndexService.get(SessionUtil.getShop_id(request));
				// mv.setViewName("/stat/indexStat");
				// mv.addObject("statIndex", statIndex);
			}
		} else {

		...
		<div class="main-content">
								...
								<div class="widget-body">
									<div class="widget-main">
										<table class="searchField" style="margin: 4px; padding: 4px;">
											<tr>
												<td>账号：</td>
												<td><input type="text" id="txtUsername" class="form-control input-small" placeholder=""
													value="${sysUserSearchVO.username }"></td>
												<td>姓名：</td>
												<td><input type="text" id="txtRealname" class="form-control input-small" placeholder=""
													value="${sysUserSearchVO.realname }"></td>
												<td>状态：</td>
												<td><form:select path="sysUserSearchVO.status" class="form-control" id="cmbStatus">
														<form:option value="" label="--全部--" />
														<form:option value="1">正常</form:option>
														<form:option value="2">已锁定</form:option>
													</form:select></td>
												<td>角色：</td>
												<td><form:select path="sysUserSearchVO.role_id" class="form-control" id="cmbRole_id">
														<form:option value="" label="--全部--" />
												...
												<td>
													...
													</button> <c:if test="${bln:isP('SysUserAdd')}">
								...
								<tbody>
									<c:forEach items="${list }" var="sysUser" varStatus="st">
										<tr>
											...
											<td><fmt:formatDate value="${sysUser.create_date }" pattern="yyyy-MM-dd HH:mm" /></td>
											<td>${bln:getUserStatus(sysUser.status)}</td>
											<td><a href="searchUserLogin.htm?id=${sysUser.id}">查看 </a></td>
			...
			<script type="text/javascript">
				...
				var lock = function(id) {
					bootbox.confirm("<h4 class='red'>你确定要锁定该用户吗？</h4>", function(result) {
						if (result) {
				...
				var unlock = function(id) {
					bootbox.confirm("<h4 class='red'>你确定要解锁该用户吗？</h4>", function(result) {
						if (result) {
				...
				var resetPass = function(id) {
					bootbox.confirm("<h4 class='red'>你确定要给该用户重置密码吗？</h4>", function(result) {
						if (result) {
